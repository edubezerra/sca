	...
	private void gravarDadosImportados() {
		...
		for (String codTurma : ofertasIt) {
			...
			try {
				...
				System.err.println("Turma nao encontrada: " + codTurma);
				turma = null;
			}
			if (turma != null) {
				...
				try {
					...
					System.err.println("Professor nao encontrado: " + turmas_docentes.get(codTurma));
				}
				if (professor != null) {
		...
		System.out.println("Foram importadas " + turmas_docentes.keySet().size() + " turmas.");
