...
public class ScaRestController {
	...
	public Map<String, String> changeCodigoDisciplina(HttpSession session, @PathVariable String codigoDisciplina)
			throws ParseException {
		Map<String, String> mapaDisciplina = new HashMap<String, String>();
		Aluno aluno = (Aluno) session.getAttribute("aluno");
		String idAluno = aluno.getId().toString();
		Disciplina disciplina = isencaoService.getDisciplinaPorCodigo(codigoDisciplina, idAluno);
		if (disciplina != null) {
			String nomeDisciplina = disciplina.getCodigo() + " - " + disciplina.getNome();
