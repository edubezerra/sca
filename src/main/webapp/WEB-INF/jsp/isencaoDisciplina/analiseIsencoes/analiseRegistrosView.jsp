...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/resources/bootstrap/js/moment.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/resources/bootstrap/js/daterangepicker.js"></script>
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/resources/bootstrap/css/daterangepicker.css" />
							...
							ranges : {
								...
								'Último Mês' : [
										...
										moment().subtract(1, 'month').endOf(
												'month') ],
								'Este ano' : [ moment().startOf('year'),
										moment().endOf('year') ],
								'Desde o início' : [ moment("01/01/2012"),
										moment() ],
							}
						}, cb);
	});
...
<script>
	function setTablesorter() {
		...
		$table1.delegate('.toggle', 'click', function() {
			...
			$(this).closest('tr').nextUntil('tr.tablesorter-hasChildRow').find(
		...
		$('button.toggle-combined').click(function() {
			var wo = $table1[0].config.widgetOptions, o = !wo.filter_childRows;
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						$("#numeroVersao").empty();
						if (siglaCurso == "") {
							$('<option>').val("").text("Todas as versões")
									.appendTo($("#numeroVersao"));
							...
							for (var i = 0; i < numeroVersao.length; i++) {
								$('<option>').val(numeroVersao[i]).text(
										numeroVersao[i]).appendTo(
										$("#numeroVersao"));
							}
...
<script>
	function promptJustificativa(idRegistro, matriculaAluno, novoStatus) {
		$.bs.popup.prompt({
			...
			atualizaStatusAtividade(idRegistro, matriculaAluno, novoStatus,
					message);
			dialogE.modal('hide');
		});
	}
	function atualizaStatusAtividade(idRegistro, matriculaAluno, novoStatus,
			justificativa) {
...
<script>
	...
	function display(data) {
		if (data.length == 0) {
			$("#lista_registros")
					.html(
							"Não há registros de atividades complementares com os critérios pesquisados!");
		} else {
			...
			for (var i = 0; i < registros.length; i++) {
				...
				table_data = table_data + "<td class='text-left'>"
						+ registros[i].nomeAluno + "</td>";
				table_data = table_data
						+ "<td class='text-left'><div style='overflow:auto;width:100%;height:40px;'>"
						+ registros[i].descrAtividade + "</div></td>";
				table_data = table_data + "<td>" + registros[i].dataSolicitacao
						+ "</td>";
				table_data = table_data + "<td>" + registros[i].dataAnalise
						+ "</td>";
				table_data = table_data + "</tr>";

				table_data = table_data + "<tr class='tablesorter-childRow'>";
				table_data = table_data
						+ "<td colspan='4'><table class='table text-center'><thead><tr>";
				table_data = table_data
						+ "<td class='text-left'><b>Descrição</b></td>"
						+ "<td><b>Carga Horária</b></td>"
						+ "<td><b>Comprovante</b></td>" + "<td><b>Status: ";
				switch (registros[i].estado) {
				case "INDEFERIDO":
					var classeStatus = "text-danger";
					break;
				case "DEFERIDO":
					var classeStatus = "text-success";
					break;
				case "EM_ANÁLISE":
					var classeStatus = "text-warning";
					break;
				case "SUBMETIDO":
					var classeStatus = "text-primary";
					break;
				}
				table_data = table_data + "<span class='"+classeStatus+"'>"
						+ registros[i].estado + "</b></span>";
				...
				table_data = table_data + "<td>" + registros[i].cargaHoraria
						+ "</td>";
				table_data = table_data + "<td>";
				...
				case "INDEFERIDO":
					table_data = table_data
							+ "<button "
							+ "onclick='atualizaStatusAtividade(\""
							+ registros[i].idRegistro
							+ "\",\""
							+ registros[i].matriculaAluno
							+ "\",\"EM_ANÁLISE\",\"\")' "
				...
				if (registros[i].estado != "SUBMETIDO") {
					table_data = table_data + "<tr>";
					table_data = table_data
							+ "<td class='text-left' colspan='4'><b>Professor Avaliador:</b> "
							+ registros[i].nomeAvaliador + "</td>";
					table_data = table_data + "</tr>";
					if (registros[i].estado == "INDEFERIDO") {
						table_data = table_data + "<tr>";
						table_data = table_data
								+ "<td class='text-left' colspan='4'><b>Justificativa:</b> "
								+ registros[i].justificativa + "</td>";
						table_data = table_data + "</tr>";
					}
				}
...
<body class="lista-solicitacoes">
	...
	<div class="container">
		<div class="row text-center">
			<h3>Análise de Pedidos de Isenção de Disciplinas</h3>
		</div>
		<hr />
		<div class="row">
			<h5>
				<b>Professor:</b>
				<c:out value="${requestScope.dadosAnaliseAtividades.nomeProfessor}"></c:out>
				(Matrícula:
				<c:out
					value="${requestScope.dadosAnaliseAtividades.matriculaProfessor}"></c:out>
				)
			</h5>
		</div>
		<br />
		...
		<div class="row">
			<form id="search-form">
				<table class="table text-center">
					<tr>
						<td><select class="form-control input" id="siglaCurso"
							onchange="getVersoesCurso()">
								<option value="">Todos os cursos</option>
								<c:forEach
									items="${requestScope.dadosAnaliseAtividades.siglaCursos}"
									var="siglaCurso">
									<option value="${siglaCurso}">${siglaCurso}</option>
								</c:forEach>
						</select></td>
						<td><select class="form-control input" id="numeroVersao"
							disabled>
								<option value="">Todas as versões</option>
						</select></td>
						<td><select class="form-control input" id="status">
								<option value="">Todos os status de registro</option>
								<c:forEach items="${requestScope.dadosAnaliseAtividades.status}"
									var="status">
									<option value="${status}">${status}</option>
								</c:forEach>
						</select></td>
						<td class="text-left">
							<div id="daterange" title="Restringir intervalo"
								class="form-control input pull-right">
								<i class="glyphicon glyphicon-calendar fa fa-calendar"></i>&nbsp;
								<span>Desde o início</span> <b class="caret"></b>
							</div> <input type="hidden" id="startDate" name="startDate" value="">
		...
		<div class="row">
			<h4>
				<b>Pedidos de isenção:</b>
			</h4>
			<div class="vcenter well" id="lista_registros">Pesquise os
				pedidos de isenção a analisar.</div>
		</div>

		<a class="btn btn-default"
			href="${pageContext.request.contextPath}/registroAtividades/menuPrincipal">
